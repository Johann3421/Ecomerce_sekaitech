...
export default function Footer() {
  const footerLinks = {
    Tienda: [
      ...
      { name: "Electrónica", href: "/category/electronica" },
      { name: "Moda", href: "/category/moda" },
      { name: "Hogar", href: "/category/hogar" },
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-12 lg:py-16 grid grid-cols-2 md:grid-cols-6 gap-8">
          ...
          <div className="col-span-2">
            ...
            <p className="text-sm text-ink-secondary leading-relaxed max-w-xs mb-6">
              Productos premium seleccionados con cuidado. Calidad, diseño y funcionalidad en cada detalle.
            </p>
            {/* Social Icons */}
            <div className="flex items-center gap-3">
              {["twitter", "instagram", "facebook", "youtube"].map((social) => (
                <a
                  key={social}
                  href="#"
                  className="w-9 h-9 rounded-lg bg-surface-100 flex items-center justify-center text-ink-tertiary hover:text-brand-500 hover:bg-brand-50 transition-all duration-200"
                  aria-label={social}
                >
              ...
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-sm text-ink-secondary hover:text-brand-600 transition-colors"
                    >
                      {link.name}
                    </Link>
                  </li>
        ...
        <div className="py-6 border-t border-surface-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-ink-tertiary">
            © {new Date().getFullYear()} LumiStore. Todos los derechos reservados.
          </p>
          <div className="flex items-center gap-3">
            {/* Payment Methods */}
...
function PaymentIcon({ type }: { type: string }) {
  return (
    <div className="h-8 px-2 py-1 bg-white rounded border border-surface-200 flex items-center justify-center">
